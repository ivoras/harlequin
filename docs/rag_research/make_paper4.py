#!/usr/bin/env python3
"""
Render paper4.html: the single-embedder selection study. Terse, data-driven.
...
Reads data/r4_agg.json (from r3_aggregate.py, RPREFIX=r4) plus lib.EMBEDDERS for the prompt
conventions, and an optional generalization file data/r4_corpus2.json. Adds a chapter on the eval-set structure (§2).
"""
import json
import math
import os
...
AGG = json.load(open(os.path.join(DATA, "r4_agg.json")))
C2 = (json.load(open(os.path.join(DATA, "r4_corpus2.json")))
      if os.path.exists(os.path.join(DATA, "r4_corpus2.json")) else None)
RRF = (json.load(open(os.path.join(DATA, "r4_rrf_sweep.json")))
       if os.path.exists(os.path.join(DATA, "r4_rrf_sweep.json")) else None)
...
TIPS = {
    ...
    "segment": "A subgroup of the in-document question set",
...
def ch_evalset():
    ...
    n = len(indoc)
    cat = lambda q: q.get("category")

    def bucket(q):                       # disjoint partition, by precedence
        if cat(q) == "exact":
            return "exact — mined"
        if cat(q) == "exact_llm":
            return "exact — named entity (LLM)"
        if q.get("misspelled"):
            return "misspelled"
        if cat(q) == "dumb":
            return "naive / 'dumb'"
        return "paraphrase (normal)"

    from collections import Counter
    b = Counter(bucket(q) for q in indoc)
    order = ["paraphrase (normal)", "naive / 'dumb'", "misspelled",
             "exact — mined", "exact — named entity (LLM)"]
    rows = [{"segment": k, "n": b[k], "pct": b[k] / n} for k in order]
    cols = [("segment", "segment", None), ("n", "n", "{}"), ("pct", "% in-doc", "{:.0%}")]
    exact_n = b["exact — mined"] + b["exact — named entity (LLM)"]
    # exact-kind breakdown from the src tag
    kinds = Counter(q.get("src") for q in indoc if cat(q) in ("exact", "exact_llm"))
    kind_txt = ", ".join(f"{k} {c}" for k, c in kinds.most_common())
    return ("<h2>2. Evaluation set</h2>\n"
            ...
            "in the text &mdash; percentages, dates, protocol numbers, "
            "article(+paragraph) references, grouped figures. <i>Named-entity</i> "
            "(100) were extracted by an LLM (treaty/instrument names, formal "
            "Member-State names, institutions &amp; bodies, terms of art, seats, "
            "dates) and kept only where the phrase occurs verbatim; grounding is "
            "duplicate-aware (acceptable = the support sentence plus occurrences "
            "that lexically overlap it). By exact-token kind: "
            f"{esc(kind_txt)}.</p>\n"
            "<p>Scoring is unchanged: a hit covers any acceptable sentence; metrics "
            "are size-normalised (<code>answer@Btok</code>) with bootstrap CIs; "
            "out-of-domain questions test rejection. The exact subset drives §9.1.</p>")


def ch_naming():
    return """<h2>3. Naming convention</h2>
...
def ch_prompts():
    ...
    return ("<h2>4. Native prompt conventions</h2>\n"
            ...
            "convention and the no-prefix mode (§5).</p>\n" + "\n".join(body))
...
def ch_prefix_gap():
    ...
    return ("<h2>5. Prompt mode: native prefixes vs no-prefix</h2>\n"
...
def ch_gate():
    out = ["<h2>6. Semantic-adjacency gate sweep</h2>",
...
def ch_chunkers():
    out = ["<h2>7. Chunker comparison</h2>",
           ...
           "shared, their ranked lists fuse directly by id under RRF (§8). A "
...
def ch_lexical():
    out = ["<h2>8. Lexical backend: BM25 vs FTS5 (+RRF)</h2>",
    ...
    for m in MODELS:
        ...
        if not lx:
            continue
        out.append(f"<h3>8.{MODELS.index(m)+1} {esc(PRETTY[m])} "
...
def ch_probe():
    ...
    return ("<h3>9.1 Exact-extraction probe and score-gated fusion</h3>\n"
...
def ch_rrf_sweep():
    ...
    return ("<h2>9. RRF weight on the lexical (FTS5) arm</h2>\n"
            ...
            "hurt typo'd ones.</p>\n"
            f"<figure>{fig}<figcaption>Misspelled queries fall off sharply as the "
            "FTS5 arm gains weight (exact-token matching cannot survive typos). The "
            "clean curve is nearly flat — it now blends paraphrase (which wants the "
            "lexical arm down) with exact-extraction (which wants it up, §9.1) — so "
            "no single global weight is right.</figcaption></figure>\n"
            ...
            + "\n<p><b>Why one global weight is wrong.</b> FTS5/BM25 is the right "
            ...
            "never drop it&rsquo;. §9.1 separates the two regimes on a dedicated "
            "exact-extraction probe and shows the weight preference inverts &mdash; "
            "motivating per-query <i>score-gated</i> fusion instead of a fixed "
            "weight.</p>"
            + ch_probe())
...
def ch_selection():
    ...
    return ("<h2>10. Model selection (composite + cost)</h2>\n"
...
def ch_generalization():
    ...
    return ("<h2>11. Generalization</h2>\n"
...
def main():
    ...
    abstract = (
        ...
        "legal corpus with 1152 grounded in-document (incl. 350 exact-extraction) and 201 out-of-domain questions, scored by "
...
<p class="fn">All figures/tables generated by <code>make_paper4.py</code> from
<code>data/r4_agg.json</code>. Pipeline: <code>r3_server.py</code> (model lifecycle),
<code>r3_build.py</code> (indexes), <code>r3_eval.py</code> (metrics),
<code>r3_aggregate.py</code> (decisions), <code>r4_run.py</code> (v4 re-eval orchestration).</p>
<script>
document.querySelectorAll('table.sortable').forEach(function(table){{
  ...
  for(var i=0;i<heads.length;i++)(function(th){{
    ...
    with open(os.path.join(HERE, "paper4.html"), "w") as f:
        f.write(html)
    print("wrote paper4.html")
