...
func (a *Agent) webFetchGrepEntry() toolEntry {
	return toolEntry{def: webFetchGrepToolDef(), handler: a.webFetchGrep}
}

func (a *Agent) webFetchGrep(ctx context.Context, rc *runContext, args map[string]any) (string, error) {
	if a.WebFetcher == nil {
	...
	if len(matches) == 0 {
		fmt.Fprintf(&sb, "No HTML matched /%s/i.\n", pattern)
		return sb.String(), nil
	}
	fmt.Fprintf(&sb, "%d match(es) for /%s/i in the page HTML (whole line if ≤%d chars, else ±%d chars of context):\n",
	...
	res := sb.String()
	if len(res) > webFetchGrepResultCap {
		res = res[:webFetchGrepResultCap] + "\n…[truncated — narrow the pattern]"
	}
	return res, nil
}
