...
- **Notifications** are **pushed** over the session WebSocket, not polled: each connection runs a server-side poller (`Server.pollNotifications`, ~5s) that reads the user's pending notifications (the DB is the store, `notify.ListPending` filtered by interface) and sends new ones as `notification` events; clients ack over REST (`POST /notifications/{id}/ack`). Clients no longer run an interval poll.
